...
namespace Tourmaline.Viewer3D
{
    ...
    public class SharedTextureManager
    {
            ...
            if (!Textures.ContainsKey(path))
            {
                try
                {
                    ...
                    if (Path.GetExtension(path) == ".dds")
                    {
                        ...
                        {
                            texture = Tourmaline.Formats.Msts.AceFile.Texture2DFromFile(GraphicsDevice, aceTexture);
                    ...
                    else if (Path.GetExtension(path) == ".ace")
                    {
                        ...
                        if (File.Exists(path))
                        {
                            texture = Tourmaline.Formats.Msts.AceFile.Texture2DFromFile(GraphicsDevice, path);
                        }
                        else
                        {
                            Texture2D missing()
        ...
        public static Texture2D Get(GraphicsDevice graphicsDevice, string path)
        {
            ...
            if (ext == ".ace")
                return Tourmaline.Formats.Msts.AceFile.Texture2DFromFile(graphicsDevice, path);
        ...
        {
            return  string.Format("{0:F0} texturas", Textures.Keys.Count);
        }
    }
    ...
    public class SharedMaterialManager
    {
        ...
        public SharedMaterialManager(Viewer viewer)
        {
            ...
            var microtexPath = viewer.Game.ResourcesPath + @"\TERRTEX\microtex.ace";
            if (File.Exists(microtexPath))
            {
                try
                {
                    SceneryShader.OverlayTexture = Tourmaline.Formats.Msts.AceFile.Texture2DFromFile(viewer.GraphicsDevice, microtexPath);
                }
                catch (InvalidDataException error)
                {
            ...
            MissingTexture = SharedTextureManager.Get(viewer.RenderProcess.GraphicsDevice, Path.Combine(viewer.ResourcesPath, "blank.bmp"));

            // Managing default snow textures
            var defaultSnowTexturePath = viewer.Game.ResourcesPath + @"\TERRTEX\SNOW\ORTSDefaultSnow.ace";
            DefaultSnowTexture = Viewer.TextureManager.Get(defaultSnowTexturePath);
            var defaultDMSnowTexturePath = viewer.Game.ResourcesPath + @"\TERRTEX\SNOW\ORTSDefaultDMSnow.ace";
        ...
        public string GetStatus()
        {
            return string.Format("{0:F0} materiales", Materials.Keys.Count);
        }
    ...
    public class SceneryMaterial : Material
    {
            ...
            : base(viewer, String.Format("{0}:{1:X}:{2}", texturePath, options, mipMapBias))
        {
            ...
            {
                var nightTexturePath = Helpers.GetNightTextureFile(Viewer.microSim, texturePath);
            ...
            if (texture.Tag != null && texture.Tag.GetType() == typeof(Tourmaline.Formats.Msts.AceInfo))
                AceAlphaBits = ((Tourmaline.Formats.Msts.AceInfo)texture.Tag).AlphaBits;
            else
                AceAlphaBits = 0;
        ...
        public override void SetState(GraphicsDevice graphicsDevice, Material previousMaterial)
        {
            ...
            //var level9_3 = Viewer.Settings.IsDirectXFeatureLevelIncluded(TOURMALINE.Settings.UserSettings.DirectXFeature.Level9_3);
            var level9_3 = true;
