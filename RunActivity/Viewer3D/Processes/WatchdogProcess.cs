...
namespace Tourmaline.Viewer3D.Processes
{
    ...
    public class WatchdogProcess
    {
        ...
        // THREAD SAFETY:
        // Hay que usar variables locales para acceder.
        // Sólo se permite crear nuevas instancias de los objetos (se pueden clonar y luego modificar), pero no
        // se autoriza hacer modificaciones a los objetos.
        ...
        /// <summary>       
        /// Registra un nuevo token de hilo de proceso para que el watchdog lo monitorice.
        /// </summary>
        /// <param name="token">El token que representa el hilo a empezar a monitorear.</param>
        public void Register(WatchdogToken token)
        {
            // Ha sido necesario hacer esta rutina elaborada porque:
            // a) No se pueden modirficar los tokens en donde están (por el peligro potencial de que otros hilos los consulten al mismo tiempo)
            // b) No se pueden asignar directamente (por el peligro potencial de que otros hilos escriban en ellos al mismo tiempo)
            while (true)
            {
                var tokens = Tokens;
        ...
        /// <summary>
        /// Elimina del registro un token para que el watchdog deje de controlarlo.
        /// </summary>
        /// <param name="token">El token que representa al hilo que se deja de controlar.</param>
