...
namespace ACadSharp
{
		...
		where T : Attribute, ICodeValueAttribute
	{
		...
		{
			if(this._property.PropertyType.GetType() != typeof(IHandledCadObject) )
		...
		{
			if(this._property.PropertyType.GetType() !=typeof(INamedCadObject))
		...
		{
			if(this._property.PropertyType.GetType()!=typeof(IEnumerable))
		...
		private object getRawValue<TCadObject>(int code, TCadObject obj)
		{
			...
			if(this._property.PropertyType.GetType() == typeof(IVector))
