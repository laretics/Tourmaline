...
namespace ACadSharp.IO.DWG
{
    internal partial class DwgObjectWriter : DwgSectionIO
    {
        private void writeEntity(Entity entity)
        {
            ...
            if(tipo==typeof(UnknownEntity) || tipo==typeof(AttributeEntity) || tipo==typeof(Solid3D) || tipo==typeof(MultiLeader)||tipo==typeof(Mesh))
            {
            ...
            else if (tipo==typeof(Circle))
            {
                Circle circulo = (Circle)entity;
                this.writeCircle(circulo);
            }
            else if(tipo == typeof(Dimension))
            {
                Dimension dimension = (Dimension)entity;
                this.writeCommonDimensionData(dimension);
                if(tipo==typeof(DimensionLinear))
                {
                    DimensionLinear dimensionLinear = (DimensionLinear)entity;
                    this.writeDimensionLinear(dimensionLinear);
                }
                else if (tipo==typeof(DimensionAligned))
                {
                    DimensionAligned dimensionAligned = (DimensionAligned)entity;
                    this.writeDimensionAligned(dimensionAligned);
                }
                else if(tipo==typeof (DimensionRadius))
                {
                    DimensionRadius dimensionRadius = (DimensionRadius)entity;
                    this.writeDimensionRadius(dimensionRadius);
                }
                else if (tipo==typeof(DimensionAngular2Line))
                {
                    DimensionAngular2Line angular2Line = (DimensionAngular2Line)entity;
                    this.writeDimensionAngular2Line(angular2Line);
                }
                else if(tipo==typeof(DimensionAngular3Pt))
                {
                    DimensionAngular3Pt angular3Pt = (DimensionAngular3Pt)entity;
                    this.writeDimensionAngular3Pt(angular3Pt);
                }
                else if(tipo == typeof(DimensionDiameter))
                {
                    DimensionDiameter diameter = (DimensionDiameter)entity;
                    this.writeDimensionDiameter(diameter);
                }
                else if(tipo==typeof(DimensionOrdinate))
                {
                    DimensionOrdinate ordinate = (DimensionOrdinate)entity;
            ...
            else if(tipo==typeof(Face3D))
            {
                Face3D face = (Face3D)entity;
                this.writeFace3D(face);
            }
            else if(tipo== typeof(Hatch))
            {
                Hatch hatch = (Hatch)entity;
                this.writeHatch(hatch);
            }
            else if(tipo ==typeof(Leader))
            {
                Leader l = (Leader)entity;
                this.writeLeader(l);
            }
            ...
            else if(tipo==typeof(Ray))
            {
                Ray ray = (Ray)entity;
                this.writeRay(ray);
            }
            else if(tipo==typeof(Shape))
            {
                Shape shape = (Shape)entity;
                this.writeShape(shape);
            }
            else if (tipo==typeof(Solid))
            {
                Solid solid = (Solid)entity;
                this.writeSolid(solid);
            }
            else if (tipo==typeof(Solid3D))
            {
                Solid3D solid3d = (Solid3D)entity;
                this.writeSolid3D(solid3d);
            }
            else if(tipo==typeof(Spline))
            {
                Spline spline = (Spline)entity;
                this.writeSpline(spline);
            }
            else if(tipo == typeof(CadImageBase))
            {
                CadImageBase image = (CadImageBase)entity;
                this.writeCadImage(image);
            }
            else if(tipo==typeof(TextEntity))
            {
                if(tipo == typeof(AttributeEntity))
                {
                    AttributeEntity att = (AttributeEntity)entity;
                    this.writeAttribute(att);
                }
                else if (tipo == typeof(AttributeDefinition))
                {
            ...
            else if(tipo == typeof(Tolerance))
            {
                Tolerance tolerance = (Tolerance)entity;
                this.writeTolerance(tolerance);
            }
            else if(tipo == typeof(Vertex))
            {
                if(tipo==typeof(Vertex2D))
                {
                    Vertex2D v2 = (Vertex2D)entity;
                    this.writeVertex2D(v2);
                }
                else if(tipo == typeof(VertexFaceRecord))
                {
                    VertexFaceRecord vface = (VertexFaceRecord)entity;
                    this.writeFaceRecord(vface);
                }
                else if (tipo ==typeof(Vertex3D)||tipo==typeof(VertexFaceMesh))
                {
            ...
            else if(tipo == typeof(Viewport))
            {
                Viewport vp = (Viewport)entity;
                this.writeViewport(vp);
            }
            else if(tipo == typeof(XLine))
            {
