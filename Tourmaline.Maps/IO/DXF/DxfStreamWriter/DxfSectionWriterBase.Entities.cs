...
namespace ACadSharp.IO.DXF
{
    internal abstract partial class DxfSectionWriterBase
    {
            ...
            where T : Entity
        {
            ...
            if(tipo==typeof(Solid3D)||tipo==typeof(MultiLeader) ||tipo==typeof(UnknownEntity))
            {
        ...
        {
            DxfClassMap map;
            Type tipo = polyline.GetType();
            if(typeof(Polyline2D)==tipo)
            {
                map = DxfClassMap.Create<Polyline2D>();
            }
            else if(typeof(Polyline3D)==tipo)
            {
                map = DxfClassMap.Create<Polyline3D>();
            }
            else if(typeof(PolyfaceMesh)==tipo)
            {
