...
#if !NEW_READER
namespace Tourmaline.Parsers.Msts
{
    ...
    public class STFReader : IDisposable
    {
        ...
        public STFReader(string filename, bool useTree)
        {
            ...
            if (Directory.Exists(path))
            {
                ...
                LineNumber = 2;
                if (useTree) mvarTree = new List<string>();
            }
            else
            {
        ...
        public string Tree
        {
            get
            {
                Debug.Assert(mvarTree != null);
                if ((mvarTreeCache != null) && (!stepbackoneitemFlag))
                    return mvarTreeCache + previousItem;
                else
                {
                    StringBuilder sb = new StringBuilder(256);
                    foreach (string t in (stepbackoneitemFlag) ? stepback.Tree : mvarTree) sb.Append(t);
                    mvarTreeCache = sb.ToString();
        ...
        private string ReadItem(bool string_mode)
        {
            ...
            if (stepbackoneitemFlag)
            {
                ...
                if (stepback.Tree != null) { mvarTree = stepback.Tree; mvarTreeCache = null; }
...
#line default
        ...
        private string UpdateTreeAndStepBack(string token)
        {
            ...
            if (token == "(")
            {
                if (mvarTree != null)
                {
                    stepback.Tree = new List<string>(mvarTree);
                    mvarTree.Add(previousItem + "(");
                    mvarTreeCache = null; // The tree has changed, so we need to empty the cache which will be rebuilt if the property 'Tree' is used
                }
            ...
            else if (token == ")")
            {
                if (mvarTree != null)
                {
                    ...
                    if (mvarTree.Count > 0)
                    {
                        mvarTree.RemoveAt(mvarTree.Count - 1);
                        mvarTreeCache = null; // The tree has changed, so we need to empty the cache which will be rebuilt if the property 'Tree' is used
                    }
                }
